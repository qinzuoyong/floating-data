 ...
 * - ADB 不可用：引导用户手动关闭
 *
 * 闪退修复：enableViaSystemSettings/disableViaSystemSettings 不再阻塞等待，
 * 打开设置页后立即返回，由 UI 层在 onResume 时刷新状态。
 * isOpeningExternal 标志防止 MainActivity.onUserLeaveHint 销毁 Activity。
 */
object KeepaliveManager {
    ...
    /**
     * 标记正在打开外部设置页面，防止 MainActivity.onUserLeaveHint 销毁 Activity
     * 在打开无障碍设置前置 true，在 MainActivity.onResume 中重置为 false
     */
    @Volatile
     ...
     * @return true=已确认成功（Shizuku静默启用）, false=需用户手动操作或失败（已打开设置页）
     */
     ...
     *
     * 修复闪退：不再阻塞等待4秒，打开设置页后立即返回 false。
     * 由 UI 层在 onResume 时通过 KeepliveA11yService.isRunning 刷新状态。
     * isOpeningExternal 标志防止 MainActivity.onUserLeaveHint 销毁 Activity。
     */
    private suspend fun enableViaSystemSettings(context: Context): Boolean {
        // 设置标志，防止 onUserLeaveHint 销毁 Activity
        ...
        // 不再阻塞等待，立即返回 false（需用户手动操作）
        Log.i(TAG, "已打开无障碍设置页，等待用户手动操作")
        return false
    }
