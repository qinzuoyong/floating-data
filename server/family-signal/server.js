 ...
 *   <- registered     { type:'registered', uid, room, peers[] }  注册回执（创建人直接收到；加入者批准后收到）
...
/** room -> { owner: uid, members: Map(uid->{ws,name,isAlive}), pending: Map(uid->{ws,name}), approved: Set(uid) } */
const rooms = new Map();

// 房间关系（owner/approved）持久化：服务器重启不丢失家庭
let saveTimer = null;
function saveRooms() {
  if (saveTimer) return;
  saveTimer = setTimeout(() => {
    ...
    for (const [room, rs] of rooms) {
      data[room] = { owner: rs.owner, approved: Array.from(rs.approved) };
    }
    ...
    switch (msg.type) {
      ...
      case 'register': {
        ...
        if (!rs) {
          ...
          rs = { owner: uid, members: new Map(), pending: new Map(), approved: new Set() };
          rooms.set(room, rs);
          saveRooms();
        }
        ...
        if (rs.owner === uid || rs.members.has(uid) || rs.approved.has(uid)) {
          ...
          send(ws, { type: 'registered', uid, room, peers: memberPeers(rs, uid) });
      ...
      case 'join-approve': {
        ...
        const name = p.name;
        // 记录已批准成员：断线重连直接进房，无需重新审核
        rs.approved.add(uid);
        saveRooms();
        rs.members.set(uid, { ws: p.ws, name, isAlive: true });
        send(p.ws, { type: 'registered', uid, room: ws.room, peers: memberPeers(rs, uid) });
