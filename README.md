...
> 版本: 1.92 | 最低支持: Android 14 (API 34)
...
────────────────────────────────────────
  ...
  v1.92  (当前版本)
          修复：加入家庭页返回取消后仍会提交——家庭码占用查询最长 15 秒才回调，用户在等待
                期间返回列表（取消）后，迟到的回调仍会写入家庭码、清空成员列表并启动共享
                服务，违背取消意图；现加组合生命周期门控，页面销毁后丢弃迟到回调
                （黑洞中继实测：改前回调如期提交，改后 15 秒窗口走满家庭码与成员均不变；
                真机同法复测通过）
          修复：撤销自动授权在主线程执行特权命令——走内置 ADB 通道时主线程 socket 触发
                NetworkOnMainThreadException（被吞后撤销全部失效，且误断已连通道）；走
                Shizuku 载体则阻塞主线程至 10 秒（ANR 风险）。现切换至 IO 调度器
                （真机实测：撤销真实生效、零 ANR、通道保持连接）
          修复：灭屏期间拉起悬浮窗服务会整夜空转采样——灭屏/亮屏广播是边沿触发，服务在
                灭屏期间被拉起（开机恢复、无障碍恢复、前台服务重投递）后不会再收到灭屏
                事件来暂停采样，2 秒轮询（含特权 shell 直读）整夜空转；现灭屏时不启动采样、
                亮屏后自动补建（真机 A/B：改前灭屏即报 30.9°C/+15.7W，改后停留占位符、
                亮屏 10 秒内回贴真实值）
          修复：家人位置共享服务遗留僵尸实例——服务未建立信令通道时被「地图」等动作拉起，
                会以 isRunning=true 的空壳驻留，短路无障碍恢复路径并让界面状态失真；现通道
                未建立即自行退出（真机 A/B：改前 ServiceRecord 滞留，改后干净销毁）
          修复：ADB 特权通道并发连接互踢引发重连风暴——三处触发点未持连接锁，并发建连会
                各自关闭对方 client；守护进程也被多路并发拉起、反复更换令牌与在途请求竞态；
                现全部持锁调用并加防重入守卫（真机 diag 日志 A/B：改前同一秒两次 CONNECTED
                并伴随多次 ping 失败与重复拉起，改后单次 CONNECTED、零重试）
          优化：以上 5 项均沉淀为源码级回归断言（npm test 107 项全绿，改前红 12 项）；
                模拟器全量走查 + vivo 真机逐项红/绿对照（含地图 WebView 渲染、逆地理编码
                落点、自愈写回主路径与巡检兜底）

  v1.91
          修复：港澳豁免区域改用真实边界——香港/澳门此前用外接矩形，而矩形在几何上无法与
                大陆分离（香港最北端 22.5591°N 高于深圳福田 22.5448°N、澳门与珠海拱北同处
                一条纬度带），导致深圳主城区与珠海拱北/横琴/湾仔被一并划入、被跳过 GCJ-02
                偏移，在百度地图上整体偏 603~620 m；现按香港北部边界折线与澳门边界多边形
                判定（真机注入已知点实测：修复前偏 605.9 m，修复后偏 0.0000 m；地图落点
                肉眼核验落在真实位置）
          修复：香港离岛北岸不再漏判——边界折线改用保守简化（把低于真实上包络的采样点全部
                补回），吉澳/东平洲一带离岛北岸不再被误施偏移（200 m 网格漏判由 5 点降为 0）
          优化：新增港澳豁免区域的源码级回归断言纳入 npm test（改前 4 项失败 → 改后 28 项通过）；
                港澳底图取向经真机实测结案（百度港澳数据接受未偏移 WGS-84，保留豁免方案）

  v1.90
          修复：ADB 经典签名通道整条失效——PKCS#1 v1.5 填充前缀多写了 44 字节（280 而非
                236），签名时两段长度之和超过模数字节数，JCA 直接抛 IllegalBlockSizeException，
                使「环回 5555 免配对直连」这条主路径与自愈基座的密钥受信校验从未成功过。
                现按模数约束修正填充长度（真机实测：日志覆盖的 7 天内环回成功记录 0 次，
                装上修复包后 2 秒内成功 3 次）
          修复：放大字号后悬浮窗越出屏幕——外观变更只失效尺寸缓存、不做位置钳位，窗口自
                原左上角向右下生长，底行文本被裁掉且用户看不到；现外观变更后按新尺寸重新
                钳位（真机实测：修复前底边越界 237px，修复后恰好贴屏幕底边）
          优化：全量代码审查（61 个 Kotlin 文件 + 原生/服务端/配置），两处修复均带改前红/
                改后绿证据并新增源码级回归断言；模拟器全量走查 + vivo 真机 A/B 验收通过

  v1.89
          修复：家人页重进不再自动重启用户已关闭的位置共享服务——每次进入家人页都会
                重跑自动启动逻辑，用户刚关掉共享、切走再切回就被静默拉起并重连信令；
                现以「应在运行」标记为门控，主动停止后保持停止，进程/设备重建后的
                自动恢复不受影响（模拟器改前复现/改后验证 + vivo 真机实测通过）
          修复：信令地址未配置时点「加入家庭」主线程崩溃——SIGNAL_URL 与备用地址均
                缺失时，家庭码占用查询会在空端点列表上抛异常；现按「查询失败」同语义
                兜底回调，走正常加入流程
          修复：无障碍通道恢复悬浮窗服务补异常兜底——个别 ROM 拦截后台启动前台服务时，
                异常会从系统回调冒泡令无障碍被判崩溃停止绑定；现与家人服务恢复路径
                一致静默降级（vivo 真机回归通过）
          优化：全量代码审查（61 个 Kotlin 文件 + 原生/服务端/配置），三处修复均带
                改前红/改后绿证据并新增源码级回归断言；服务器端停用测试桩并清理
                测试房间与测试成员

  v1.88
          优化：家人位置共享不再依赖悬浮窗保活——服务改由系统无障碍绑定维持进程不进入
                cached 类（实测进程状态为 BOUND_FOREGROUND_SERVICE，非 cached），
                不开悬浮窗也能持续被家人请求到位置，且不新增任何常驻通知
          新增：进程/设备重建后自动恢复位置共享——无障碍保活通道与开机广播两条独立路径，
                无需再手动打开家人页；标记只在用户主动关闭时清除，系统回收进程不影响
          修复：无障碍自愈漏判「服务被系统判为崩溃」——应用真崩溃后系统会把无障碍服务
                列入 crashed 并停止绑定，而它仍留在启用列表里，自愈据此判为健康、永不重绑，
                保活会静默失效；现改为按绑定实况复核并强制重绑，同时新增进程启动触发点
                （此前「真崩溃 + 未开悬浮窗」组合下自愈没有任何触发时机）
          优化：首页「无障碍保活」说明补充家人位置共享

  v1.87
          新增：家人位置共享支持主/备信令服务器自动切换——主服务器连续 3 次握手失败
                即切到备用服务器（实测约 15 秒完成注册进房），主服务器恢复后每 120 秒
                回探一次、探通即自动切回（实测约 2 秒）。只有「握手都没成功」才判为
                服务器不可达，服务器可达但被限流拒绝不会误切；生效服务器在应用内共享，
                保证家人服务与「查家庭码」落在同一台服务器上。备用地址经 local.properties
                的 SIGNAL_URL_BACKUP 配置，未配置即维持单服务器行为
          修复：家人共享信令限流按真实客户端 IP 分桶——此前经 nginx 反代后服务器只看到
                回环地址，全服用户共用一个限流桶，单个客户端刷满配额会让其他家庭在
                同一个 60 秒窗口内被连坐限流

  v1.86
          修复：常驻通知变占位——服务每次被唤醒（15 分钟心跳、无障碍退位补启等）都会用
                占位内容重建通知，而监控器只在自己缓存越过阈值时才刷新，温度稳定时
                通知会长期停在「--°C / --W」；现唤醒后立即贴回真实数值
          修复：家人页「创建 / 加入家庭」按系统返回键会直接退出应用（并从最近任务移除）
                ——现与左上角返回箭头一致，回到家人列表；返回箭头也不再顺手启动
                位置共享服务
          修复：ADB 自愈基座密钥受信写入带单引号——内置常驻服务把参数原样写入 adb_keys，
                带引号写入的是非法公钥行，设备无法据以受信，预埋信任长期静默失效
          修复：环回直连（重启后的主路径）不再跳过常驻服务拉起——此前该路径不拉起内置
                特权服务、也不推进自愈基座，一旦 ADB 会话结束特权数据静默退回基础档
          修复：特权通道降级链——内置服务存活但单条命令失败时，不再直接把失败交给调用方，
                按设计继续尝试内置 ADB 通道
          修复：家人信令注册被服务器拒绝（限流 / 房间数达上限）后永久卡在「正在连接」
                ——现主动断开并退避重连，限流窗口过后自动恢复
          修复：高德定位源异常未收敛——SDK 构造或启动抛异常会让应用崩溃（家人位置请求
                是远端可触发路径），现按「无结果」处理，静默退回系统定位
          修复：服务器重启后误删空闲家庭——回收逻辑把「无活跃记录」当成从未活跃，
                重启后第一次回收就会删掉只有创建人的家庭；现从进程启动时刻起算 7 天
          修复：家人备注编辑取消后残留文本，会在下次打开时冒充内容并可能被误保存
          修复：更新检查在全部数据源失败时谎报「已是最新版本」；「稍后安装」不再删掉
                已校验的安装包（此前关掉弹窗即删除，用户需重新下载）
          修复：屏幕旋转后回到首页 Tab、已填写的家庭码被清空；关于页「忽略电池优化」
                按钮文案在系统设置页返回后不刷新
          修复：配对握手加超时（对端半开时此前会永久占住线程与 native 上下文）；
                ADB 帧长加校验上限，畸形数据流不再触发超大内存分配
          优化：锁屏隐藏一致性——通知刷新也带 VISIBILITY_SECRET，此前首次刷新后
                常驻通知会出现在锁屏上

  v1.85
          修复：特权通道并发串包——共享 ADB 连接上的 shell 命令加串行锁；此前连接
                成功瞬间的自动授权、内置服务拉起与电池采样并发执行，会在同一条
                ADB 连接上交错收发，导致输出串包与误判断线重连
          修复：家人位置请求去重串扰——「本机发出的请求」与「对方来请求的去重」
                分表记录；此前本机刚请求过对方位置时，对方反向发来的请求会被
                误判为重复而静默丢弃（双方先后打开彼此地图页的真实场景）
          修复：家人共享服务销毁后连接状态滞留——UI 此前会一直显示「已连接」，
                现销毁时复位为未连接，与服务实际存活状态一致
