...
> 版本: 1.75 | 最低支持: Android 14 (API 34)
...
────────────────────────────────────────
  ...
  v1.75  (当前版本)
          新增：ADB 高精度数据源配对一次、后续免重复配对——开关关→开直接重连
                （不再弹出配对引导）；无线调试关闭再开启后 NSD 漫游自动恢复，
                设备上已授权的密钥在无线调试重开后依然受信任（Shizuku 同款策略）
          新增：设备撤销信任检测——TLS 握手失败判定为已撤销授权，自动暂停重连
                并给出「重新配对」入口
