...
> 版本: 1.84 | 最低支持: Android 14 (API 34)
...
────────────────────────────────────────
  ...
  v1.84  (当前版本)
          修复：自动授权撤销改为读回复核——撤销后逐项验证实际状态，仅对确认生效的
                条目移除记录，失败项保留在卡片中可重试并明确提示；此前撤销失败也会
                报成功并清空记录，用户误以为已撤销而权限仍然生效
          修复：ADB 自愈基座 trust-key 时序——密钥异步初始化尚未就绪时不再置
                「已尝试」标记，避免公钥从未写入而该设备此后永不再尝试受信
          说明：家人加入审核当前为【停用】状态——新成员输入正确家庭码后直接进房，
                无需创建人批准。因此家庭码即等同于访问凭据，请勿外泄；服务端已对
                家庭码查询与位置请求限流。恢复审核需修改 server/family-signal/server.js
                的 APPROVAL_DISABLED 常量，并与客户端审核 UI 同步（客户端逻辑已保留）

  v1.83
          安全：曾恢复家人加入审核（该模式后续已停用，见 v1.84 说明）——新成员需
                创建人批准，阻断「枚举家庭码即可进房查看家人位置」的隐私泄露
                （此前 v1.81 曾停用审核）
          安全：信令链路加固——房间/位置请求限流、位置载荷白名单校验、客户端对远端
                数据异常隔离，防伪造位置与幽灵成员注入
          安全：地图页 HTML 注入修复——家人备注名统一转义，并移除网页可反向读取
                位置的原生桥接口
          安全：APK 更新下载加签名证书校验与逐跳域名白名单，防中间人替换安装包
          修复：ADB 自愈基座 trust-key 写入逻辑（原「文件存在即不写」且双重编码，
                导致密钥受信静默失效）
          修复：ADB 密钥初始化移出主线程——消除冷启动阻塞与 ANR 风险
          优化：ADB 自动授权透明化——首页展示「已自动授予的权限」并支持一键撤销
          修复：ADB 通道读超时下沉到 socket 层、无障碍自愈退避改用系统时钟、
                滑块拖动结束才落盘、allowBackup 关闭等多项健壮性与体验修复

  v1.82
          优化：灭屏暂停电池采样——悬浮窗与通知不可见时停止 2 秒轮询（含特权通道
                读取），亮屏立即恢复，大幅降低后台耗电，减少厂商「后台高耗电」
                连带清理对微信等后台应用的影响
          优化：通知刷新 60 秒节流——功耗波动不再触发高频刷新，节流窗口过后
                自动补刷，数据不丢
          优化：悬浮窗文本去重——数值未变跳过重绘，消除无效重排
