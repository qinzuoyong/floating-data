...
> 版本: **1.51** | 最低支持: **Android 14 (API 34)**
...
| 版本 | 日期 | 变更 |
|------|------|------|
| v1.51 | 2026-06-16 | 🚀 **性能优化**：Shizuku 反射缓存（减少 CPU 反射开销）；协程调度优化（数据计算不占主线程）；Intent 注册合并（每 2 秒减少 1 次 I/O 调用）；锁定状态内存缓存（拖拽时不再每帧读 SharedPreferences）；视图尺寸缓存（clamp 时避免重复 measure）；通知阈值去重（温度±0.1°C/功耗±0.1W 以上才更新通知） |
